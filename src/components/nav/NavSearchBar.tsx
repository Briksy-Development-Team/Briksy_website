...
export const NavSearchButton = ({ setMode, hasHero = true }: Props) => {
  ...
  return (
    <button
      ...
      className={`${BTN} h-11 w-11 rounded-full shadow-md`}
...
export const NavSearchPanel = ({ mode, setMode }: Props) => {
  ...
  useEffect(() => {
    const close = (e: MouseEvent) => {
      if (filterOpen || voiceOpen) return; // don't collapse while a modal is open
  ...
  return (
    ...
    >
      {/* Left slot — back-to-search button, slides in for AI mode */}
      <div
      ...
      >
        <button
          onClick={() => setMode("ai")}
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white shadow-xl transition hover:bg-gray-100 sm:h-12 sm:w-12 lg:h-14 lg:w-14"
        >
          <Sparkles size={20} />
        </button>
      </div>
